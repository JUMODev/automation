...
class AutomationRecord(models.Model):
    ...
    def _search(self,
        ...
        # Llama al método _search de la clase base sin el argumento count
        ids = super()._search(
            args,
            offset=offset,
            limit=limit,
        ...
        # Procesa la búsqueda para encontrar registros faltantes
        if not ids:
        ...
        ids = set(ids)
        result = []
        model_data = defaultdict(lambda: defaultdict(set))  # {res_model: {res_id: set(ids)}}
        ...
        if len(orig_ids) == limit and len(result) < len(orig_ids):
            result.extend(
                self._search(
                    ...
                    limit=limit,
                    order=order,
                    count=False,  # No necesitas pasar count aquí
                    access_rights_uid=access_rights_uid,
                )[: limit - len(result)]
            )
